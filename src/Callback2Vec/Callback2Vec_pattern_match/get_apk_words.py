...
reader=csv.reader(open('/homeamd_data_pattern/pattern/amd_key_words_new.txt'))
all_key_words=[]
for k in reader:
    if k !=[]:
        all_key_words.append(k[0])
reader=csv.reader(open('/home/callback_api.csv'))
all_callback=set()
for i in reader:
    all_callback.add(i[1])

def get_data():
    path='/home/amd_data_pattern/pattern/patterns/'
    all_data=[]
    data_corpus=[]
    patterns=[]
    ...
    for path in paths:
        ...
        for i,pattern in enumerate(all_data):
            ...
            for pattern_key in pattern:
                pattern_vector=get_method_vector(pattern_key)
                for k in all_apk_data:
                    vector=get_method_vector(k)
